  ...
  switch (line) {
    ...
    case 1: {
      ...
      do {
        ...
        const std::string pad(0, ' ');

        if (wd != firstdow) os << std::string(4, ' ');
    ...
    for (auto line{0u}; line < lines; ++line) {
      for (auto col{0u}; col < cldMonCols_; ++col) {
        ...
        if (cldMode_ == CldMode::YEAR || y / mon == ym) {
          ...
          if (line == 0 && col != 0 && (col%2 == 1) && cldMode_ == CldMode::YEAR) os << std::string(2, ' ');
          else if (col != 0 && cldMode_ == CldMode::YEAR) os << std::string(3, ' ');
          ...
          if (line == 1 && col > 0 && cldMode_ == CldMode::YEAR) os << std::string(16, ' ');
